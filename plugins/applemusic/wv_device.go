package applemusic

import "encoding/base64"

// Built-in Widevine L3 device credentials for zero-config native DRM
// decryption. These are the public, shared test-device credentials used by
// apple-music-downloader and similar tools — they are NOT account-specific
// secrets, so embedding them here is safe and lets decryption work out of the
// box without requiring the user to supply any .wvd / device files.
//
// Users can still override with their own L3 device via the wv_client_id /
// wv_private_key config options (see register.go: loadWVDevice).

const defaultWVPrivateKeyPEM = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`

const defaultWVClientIDBase64 = ""

// defaultWVCredentials returns the built-in Widevine L3 device credentials:
// the decoded client ID blob and the RSA private key PEM bytes.
func defaultWVCredentials() (clientID, privKey []byte) {
	clientID, _ = base64.StdEncoding.DecodeString(defaultWVClientIDBase64)
	privKey = []byte(defaultWVPrivateKeyPEM)
	return
}
