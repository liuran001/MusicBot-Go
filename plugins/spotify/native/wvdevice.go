package native

import (
	"encoding/base64"
	"fmt"

	widevine "github.com/iyear/gowidevine"
)

// Built-in Widevine L3 device credentials for zero-config native DRM
// decryption of Spotify's MP4/AAC (CENC) streams. These are the public, shared
// test-device credentials used by apple-music-downloader, pywidevine examples,
// and similar tools — they are NOT account-specific secrets, so embedding them
// here lets decryption work out of the box without the user supplying a .wvd.
//
// They are the same credentials the applemusic plugin embeds. Operators can
// override with their own L3 device via config (see register.go).
//
// NOTE: a shared public L3 device may be revoked by the license server at any
// time; if Widevine license requests start failing with a device error, the
// operator should supply a fresh .wvd.

const defaultWVPrivateKeyPEM = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`

const defaultWVClientIDBase64 = ""

// defaultWVCredentials returns the built-in Widevine L3 device credentials:
// the decoded client ID blob and the RSA private key PEM bytes.
func defaultWVCredentials() (clientID, privKey []byte) {
	clientID, _ = base64.StdEncoding.DecodeString(defaultWVClientIDBase64)
	privKey = []byte(defaultWVPrivateKeyPEM)
	return
}

// BuiltinWidevineDevice builds a gowidevine L3 device from the embedded
// credentials. Returns an error if the credentials fail to parse.
func BuiltinWidevineDevice() (*widevine.Device, error) {
	clientID, privKey := defaultWVCredentials()
	dev, err := widevine.NewDevice(widevine.FromRaw(clientID, privKey))
	if err != nil {
		return nil, fmt.Errorf("native: load builtin widevine device: %w", err)
	}
	return dev, nil
}
