...
func (h *ChosenInlineMusicHandler) handleChosenTrack(ctx context.Context, b *telego.Bot, chosen *telego.ChosenInlineResult, platformName, trackID, qualityValue string) {
	...
	withInlineMessageLock(chosen.InlineMessageID, func() {
		...
		editInlineMedia := func(songInfo *botpkg.SongInfo) (bool, error) {
			...
			if resolveForwardButtonEnabledForUser(ctx, h.Music.Repo, chosen.From.ID) {
		...
		songInfo, err := h.Music.prepareInlineSongWithTimeout(ctx, b, chosen.From.ID, chosen.From.Username, platformName, trackID, qualityValue, progress)
		if err != nil {
			if h.Music.Logger != nil {
